...
def run(fastq_directory, mapping_file):
    ...
    barcodes = []
    for record in SeqIO.parse('promoter_library.fasta', 'fasta'):
    ...
    print("{} total files to process: ".format(samples.shape[0]))
    ...
    for index, row in samples.iterrows():
        ...
        command = "vsearch --search_exact {base}.fasta --db promoter_library.fasta --blast6out {base2}.blast"
        command = command.format(base=base, base2=base2)
        output = subprocess.check_output(
            command, shell=True, stderr=subprocess.STDOUT
        ).decode()
        for line in output.split("\n"):
